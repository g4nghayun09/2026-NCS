...
- 실습 : (19_array_object.html)
  ...
  ⑥ unshift() : 배열의 맨 앞에 요소를 추가하고 배열의 길이를 반환 => 실무에서 많이 사용.
      ...
      //실무활용 => 음료를 데이터베이스에서 가져와서 일정한 기준으로 분류하여 활용한다고 가정.

  ⑦ splice(1, 2, 3) : 배열의 특정 위치레 새로운 요소 추가
  ...
  ⑬ reduce(p1, p2, p3, p4) : 배열요소의 누적 합을 구하는 함수, 또는 특정 조건에 해당하는 값만 추출하는 함수.
...
1. alert() : 모양 관계로 거의 사용하지 않는다. => 경고 창
2. prompt() : 모양 관계로 거의 사용하지 않는다. => 문자를 입력받는 창
3. confirm( ) : 모양 관계로 거의 사용하지 않는다. => yes or no를 진행하는 창
4. open() : 모양 관계로 거의 사용하지 않는다. => 새 창 열기
